...
def read_json(path_to_jsonfiles):
    ...
    for idx, file in enumerate(files):
        ...
        if idx == 5 and 'evaluation' in file: break
        full_filename = file
...
def nohue_append(tt, ts, folders, args):
    ...
    # append_s_t(tt, ts, folders, nohue_args, back_tr=True, swap_train_test=True)
    return nohue_args
...
def nopfr_append(tt, ts, folders, args):
    ...
    # append_s_t(tt, ts, folders, nopfr_args, back_tr=True, swap_train_test=True)
    return nopfr_args
...
append_s_t(tt, ts, folders, default_args, back_tr=False, swap_train_test=True)

folders = ['evaluation', 'test']
# folders = ['evaluation']
...
append_s_t(tt, ts, folders, default_args, back_tr=False, swap_train_test=True)
# append_s_t(tt, ts, folders, default_args, back_tr=True, swap_train_test=True)
...
# append_s_t(tt, ts, folders, to_args, back_tr=True)swap_train_test
append_s_t(tt, ts, folders, to_args, back_tr=False, swap_train_test=True)
# append_s_t(tt, ts, folders, to_args, back_tr=True, swap_train_test=True)
