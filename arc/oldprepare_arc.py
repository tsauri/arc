...
train_dicts=read_json('training')
    ...
    for idx, d in enumerate(dicts):
        ...
        for f in test:
            ...
            for l in f['input']:
                src.extend(l + [NR,])
            src.extend([EOI])
            srcs.append(copy.deepcopy(src)) # we need to change src if not end seq
            if len(test) >1:
                print(idx, len(test))
                break
            ...
            # if len(test) >1:import pdb;pdb.set_trace()
            if len(test) >1 and f is not test[-1]: 
                src.extend(tgt)
    with open(filename+".src.txt", "w") as src_text:
        for src in srcs:
            src = [str(i) for i in src]
            src_text.write(' '.join(src))
            src_text.write("\n")
    with open(filename+".tgt.txt", "w") as tgt_text:
        for tgt in tgts:
...
# gen_seq('evaluation','arc_raw/dev')
